            ...
            try {
                if (file.createNewFile()) {
                    if (mc.player != null) {
                        ...
                        MsgUtil.sendModuleMsg("Open your meteor-client folder to edit autosign.txt.", this.name);
                    }
                }
            } catch (Exception err) {
            ...
            try {
                if (file.createNewFile()) {
                    if (mc.player != null) {
                        ...
                        MsgUtil.sendModuleMsg("Open your meteor-client folder to edit storysign.txt.", this.name);
                    }
                }
            } catch (Exception err) {
                LogUtil.error("Failed to create " + file.getAbsolutePath() + "! Why:\n" + err, this.name);
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (timer >= 5) {
            ...
            if (!signsToWax.isEmpty()) {
                ...
                // No-op: we no longer manage WaxAura here to avoid missing dependency.
