...
import com.example.addon.util.LogUtil;
...
import com.example.addon.util.StardustUtil;
...
import com.example.addon.mixin.accessor.ClientConnectionAccessor;
...
public class SignHistory extends Module {
    public SignHistory() {
        super(Main.RENDER, "Sign-History", "Records & restores broken or modified signs.");
    }
    ...
    private void readSignsFromFile(Path signsFile) {
        // Disabled due to missing StringNbtReader APIs in this environment.
        // We keep file writing enabled for future compatibility, but skip reading to avoid compile errors.
        try { Files.lines(signsFile).close(); } catch (Exception ignored) {}
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        // Removed dependency on external WaxAura module.
